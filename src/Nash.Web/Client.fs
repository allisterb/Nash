...
module Client =
    ...
    let mutable CUI = {
        ...
        Avatar = 
            ...
            web.Version <- 8.5
            web.Connection <- sdk
            web.Avatar <- "20031372" // "20926221" // "20053084"
            web.Voice <- "cmu-slt";
    ...
    let Main =             
        ...
        let mainOpt =
            Options(
                Name="Main", 
                Greetings = "Welcome to Nash. Enter 'hello' or 'hello my name is...(you) to initialize speech.",
                Prompt = "&#x2328;  " //&#x1f4ac;
            )       
    ...
    let run() =        
        ...
        CUI.EchoDoc <| Doc.Concat [
            Bs.btnPrimary "hello" (fun _ _ -> initMic(main');initSpeech();quick_utter "greet" |> push |> Nash.Web.Main.update)
            Html.text "     "
            Bs.btnInfo "help" (fun _ _ -> initMic(main');initSpeech();quick_utter "help" |> push |> Nash.Web.Main.update)
        ]
        Doc.Empty
